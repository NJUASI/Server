...
public class WebMarketerDataHelperImpl implements WebMarketerDataHelper{
	 ...
	 */
	public ResultMessage add(WebMarketerPO webMarketerPO) {
	 ...
	 */
	public ResultMessage delete(String webMarketerID) {
	 ...
	 */
	public WebMarketerPO getSingle(String webMarketerID) {
	...
	public List<WebMarketerPO> getAll() {
		...
		List<WebMarketerPO> list  = new ArrayList<WebMarketerPO>(); //封装多条数据
		...
		try {
			...
			while(rs.next()){
				WebMarketerPO map =  new WebMarketerPO(rs.getString(1),rs.getString(2)); //封装一条数据
				list.add(map);
			}
